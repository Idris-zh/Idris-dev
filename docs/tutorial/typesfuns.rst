...
Idris 定义了一些原语类型：``Int``、``Integer`` 和 ``Double`` 用于数值类型，
``Char`` 和 ``String`` 用于文本操作，``Ptr`` 则表示外部指针。库中还声明了一些数据类型，
包括 ``Bool`` 及其值 ``True`` 和 ``False``。我们可以用这些类型来声明常量。
将以下内容保存到文件 ``Prims.idr`` 中，并在命令行中输入 ``idris Prims.idr``，
将其加载到 Idris 的交互式环境中：
...
Idris 文件由一个可选的模块声明（此处为 ``module Prims``）后跟一个可选的导入列表，
和一组声明及其定义构成。在本例中并未指定导入。然而 Idris 可由多个模块构成，
每个模块中的每个定义都有它自己的命名空间。这一点会在 :ref:`sect-namespaces`
一节中进一步讨论。在编写 Idris 程序时，给定声明的顺序和缩进都很重要。
函数和数据类型必须先定义再使用，而且每个定义都必须有类型声明，例如之前列表中的
...
我们可以在提示符中使用它们。在提示符中求值会给出一个答案及其的类型。
例如：
...
在对 ``List`` 的声明中，我们使用了中缀操作符 ``::``。
像这样的新操作符可通过缀序声明来添加，如下所示：
...
函数通过模式匹配实现，语法同样和 Haskell 类似。其主要的不同在于 Idris
的所有函数都需要类型声明，使用单冒号 ``:`` （而非 Haskell 的双冒号 ``::`` ）。
某些自然数的运算函数像下面这样定义，同样来自于标准库：
...
并使用上面的函数来定义。和 Haskell 不同，类型和函数名的字首字母并无大小写限制。
函数名（前面的 ``plus`` 和 ``mult`` ），数据构造器（``Z``、``S``、``Nil`` 和 ``::``）
以及类型构造器（``Nat`` 和 ``List``）均为同一命名空间的一部分。然而按照约定，
...
.. note::
   ...
   当显示一个 ``Nat`` 元素，如 ``(S (S (S (S Z))))`` 时，Idris 会将其显示为
...
当然，你可能会很好奇，既然计算机已经完美地内建了整数运算，
我们为什么还需要一进制自然数？其原因主要在于一进制数有着非常便于推理的结构，
而且易与其它数据结构相关联，之后我们就会看到。尽管如此，
我们并不希望以牺牲效率为代价获得这种便捷。幸运的是，Idris 知道
``Nat`` （以及类似的结构化类型）和数之间的关系。这意味着它可以优化其表示和函数，
如 ``plus`` 和 ``mult``。
...
函数也可通过 ``where`` 从句来 **局部** 地定义。例如，要定义一个反转列表的函数，
我们可以使用一个辅助函数来累加新的，反转后的列表，并且它无需全局可见：
...
缩进是十分重要的，``where`` 块中函数的缩进必须比外层函数更深。
...
.. note:: 作用域
    ...
    任何在外部作用域中可见的名字，在 ``where`` 从句中也可见（除非它们被重新定义过，
    例如这里的 ``xs``）。若某个名字是某个类型的 **形参**，那么仅当它在类型中出现时才会在
    ``where`` 从句的作用域中，即，它在整个结构中是固定不变的。
...
除函数外，``where`` 块也可包含局数据声明，如以下代码中的的 ``MyLT``
就无法在 ``foo`` 的定义之外访问。
...
通常，在 ``where`` 从句中定义的函数和其它顶层函数一样，都需要类型声明。
...
Idris 程序中可以挖 **坑** 来表示未完成的部分。例如，我们可以在「Hello world」
...
语法 ``?greeting`` 挖了一个坑，它表示程序中尚未写完的部分。这是个有效的 Idris
...
检查坑的类型也会显示作用域中任何变量的类型。例如，给定一个 ``even`` 的未完成定义：
...
我们可以检查 ``even_rhs`` 的类型并查看期望的返回类型，以及变量 ``k`` 的类型：
...
在 Idris 中，类型是一等的，即它们可以像其它语言构造那样被计算和操作（以及传给函数）。
例如，我们可以编写一个计算类型的函数：
...
该函数可从一个 ``Bool`` 值计算出适当的类型，该布尔值表示类型是否为一个单例。
我们可以在任何可以使用类型的地方用该函数计算出一个类型。例如，它可用于计算返回类型：
...
依赖类型的一个标准例子就是「带长度的列表」类型，在依赖类型的文献中，
它通常被称作向量。向量作为 Idris 库的一部分，可通过导入 ``Data.Vect``
来使用，当然我们也可以像下面这样声明它：
...
``Vect`` 这样的依赖类型定义函数。 ``Vect`` 上函数的类型能够描述所涉及的向量
其长度会如何变化。例如，下面定义的 ``++`` 用于连接两个 ``Vect``：
...
该错误信息指出了两个向量之间的长度不匹配：我们需要一个长度为 ``k + m`` 的向量，
但提供了一个长度为 ``k + k`` 的向量。
...
从它的签名中，我们可以看到这个类型构造器接受一个 ``Nat``，然后产生一个 **类型** 。
因此，它不是一个「对象的容器」意义上的集合，而是一个未命名元素的一般集合，举例来说，
...
它表示该集合中的元素个数。由于我们无法构造出一个空集的元素，因此也就无法构造出
``Fin Z``。
...
如之前提到的， ``Fin`` 家族的用处之一在于表示有界的自然数。由于前 ``n``
...
它接受两个参数：一个  ``n`` 元素有限集的元素，以及一个类型为 ``a`` 的  ``n``
...
你写不写它纯属偏好问题，不过有时它也能让参数更加明确，有助于函数文档的记录。
...
此外， ``{}`` 可在左边被用作模式匹配，即 ``{var = pat}`` 获取一个隐式变量并试图对
「pat」进行模式匹配。例如：
...
有时提供隐式参数的类型很有用，特别是存在依赖顺序，或隐式参数本身含有依赖的情况下。
例如我们可能希望在下面的定义中规定隐式参数的类型，它为向量定义了谓词（它也在
...
``IsElem x xs`` 的实例规定了 ``x`` 是 ``xs`` 中的一个元素。我们可以构造这样的谓词，
...
然而，此限制可通过使用 ``mutual`` 互用块来放松，它允许数据类型和函数同时定义：
...
我们先无视 ``IO`` 抽象的定义，不过它有效地描述了被执行的 I/O 操作是什么，
...
该函数使用参数 ``t`` 或 ``e`` 之一，而非二者都用（我们之后会看到它其实被用来实现
``if...then...else`` 构造）。我们更希望 **只有** 用到的参数才被求值。为此，
Idris 提供了 ``Lazy`` 数据类型，它允许延迟求值：
...
余数据类型通过将递归参数标记为潜在的无穷来定义无穷数据结构。对于一个余数据类型
...
要重点注意 codata 不允许创建无穷互用递归数据结构。
例如以下代码会创建一个无穷循环并导致栈溢出：
...
不会将它添加到与正在定义的类型 **不同** 的构造器的参数上。例如，以下程序输出「1」。
...
上面的表达式其实还有更加利落的写法。其中一种就是使用匿名函数：
...
或 ``MkPair a b``。元组可包含任意数的值，表示为嵌套的序对：
...
Idris 区分 **完全** 函数与 **部分（偏）** 函数。全函数满足以下情况之一：
